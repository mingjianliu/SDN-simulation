...
////////////////////////////////////////////////
std::map<int,int> ingress;
std::map<int,int> outgress;
                    ...
                    /*here the number is used for hard code, should be deleted afterward*/)
{
    ...
    string filename1="ingress";
    ofstream fout(filename1.c_str(),ofstream::out | ofstream::app);
    //fout<<"totally "<<numberofnodes<<" of nodes"<<endl;
    for( std::map<int,int>::iterator cit=ingress.begin() ; cit != ingress.end() ; ++cit){
        fout<<(cit->first)<<"\t"<<(cit->second)<<endl;
        nodes_counter[cit->second]++;
    }
    
    fout<<"----------------------stastical--------------------"<<endl<<endl;
    for( std::map<int,int>::iterator cit=nodes_counter.begin() ; cit != nodes_counter.end() ; ++cit) {
    	fout<<(cit->first)<<"\t\t"<<(cit->second)<<endl;
    }   
...
//----------------------------build up topology-----------------------------
    ...
    path_statistic(0,numberofnodes); 
